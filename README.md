...
- **(New in Iter11)** 放弃基于 Loss 惩罚的硬约束与 EDL（证据深度学习），彻底转向 **Ansatz 架构级物理硬接线（Physics-Informed Ansatz Hard-Wiring）**。通过引入不可学习的物理距离门控函数 $D(t)$，在网络拓扑结构中直接阻断高噪数据引起的梯度冲突与 B样条网格畸变，释放 KAN 的非线性表达能力。
- 当前主干强调多周期输入、局部触发锚点、基于 SSM/门控的共享状态约束与无惩罚型训练。
...
截至 `2026-04-22`，仓库中最值得关注的工作线包括：

- **(Iter14)** **期望值回归 (Expected Value Regression)** 架构重构：彻底废弃基于二分类与指数级 MAE 惩罚的离散目标生成逻辑，将模型目标从“预测胜率”转变为“预测连续期望收益”。采用非对称 Huber Loss 处理极度不平衡的时序信号，并移除了 Sigmoid 输出限制，让模型自然展现捕捉高 EV 交易机会的能力。
- **(Iter13/Iter12)** 移除了阻碍梯度流动的硬接线物理门控，转向软门控与特征融合；引入 Focal Loss 解决极度不平衡分类任务，为回归任务的转型铺平了架构基础。
