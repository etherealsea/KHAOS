...
## 2026-04-22 最新开发进度 (Iter14: 期望值回归重构)

### 1. 彻底废弃指数惩罚与二分类逻辑
在之前的迭代 (Iter12/Iter13) 中，虽然修复了梯度消失和网络死区，但模型陷入了极度不平衡任务中的局部最优——“概率坍塌”（输出绝大多数为非事件以获取高 Accuracy）。其根源在于：
- **MAE 指数级惩罚**：`np.exp(-1.2 * mae_norm)` 导致大量本该被学习的信号目标趋近于 0。
- **Top 5% 硬截断**：将连续信号强行截断为 0/1，丢失了大量中间价值的分布信息。

在 **Iter14** 中，数据端 (`data_loader.py`) 的目标生成逻辑已重构为**连续期望值 (Expected Value, EV) 回归**：
- `score = mfe_norm - 1.2 * mae_norm`，允许目标值为负，且保留完整的连续分布特征。

### 2. 神经网络架构与损失函数升级
为了适配连续 EV 目标，我们进行了深度重构：
- **移除 Sigmoid 输出限制 (`kan.py`)**：最后一层的激活函数从 `torch.sigmoid()` 替换为 `10.0 * torch.tanh(x / 5.0)`，使模型可以直接输出 `[-10, 10]` 的连续收益预期分数，而不再是受限的概率值。
- **引入非对称 Huber Loss (`loss.py`)**：完全移除了用于二分类的 Focal Loss。改用 `F.smooth_l1_loss`，并且引入了**非对称惩罚机制**：当真实目标 $EV > 0$ 且模型预测偏低（即漏报高价值信号）时，给予双倍梯度惩罚，强制模型主动去捕捉盈利机会。

### 3. 评估与阈值系统适配
- **连续域自适应阈值 (`train.py`)**：评估脚本移除了对输出进行 `[0, 1]` 强制截断的逻辑，允许评估系统利用分位数直接在 EV 分数域上搜寻触发阈值。
- **Structural Quality 缩放修正**：取消了对 `directional_floor` 使用 `label_frequency` 盲目放大的机制，因为现在它代表真实的 EV 期望，而非极小概率。
...
- 目标是把当前研究与工程进度固化为可提交材料：目标-方法-工程-结果-风险-计划。
- 主引用基线以 multi-asset 的 `iter9` 为核心，A 股与平台作为风险与经验教训补充。
- 交付物位置：
  [`Finance/04_项目文档/01_学术论文`](Finance/04_项目文档/01_学术论文)

### P0：多资产回测对比 + 报告产物（主交付路径）

- 目标是形成可复现的多资产回测闭环，用报告产物驱动下一阶段迭代。
- 平台侧（TradingView / 同花顺）定位为可选验证路径，不再作为当前阶段主阻塞点。

### P2：A 股专用 teacher 训练（保留，但不作为当前主交付）
