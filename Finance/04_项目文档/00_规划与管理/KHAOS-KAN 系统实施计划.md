...
* 价格行为标注（弱监督）：
  ...
  * 震荡：|H\_centered|接近0、穿越频繁但非极端集中、延续一般。

* 可选：无监督HMM得到三类隐含状态，再用价格行为规则对齐为TR/MR/Chop。

## 特征工程

* Hurst四路径估计：GHE(去均值)、DFA、Haar/二次变差、改良R/S；参数：窗口组(H\_len1/2/3)、更新周期(H\_update\_every)、收缩γ映射常数。
...
* H组：H\_len1/2/3（每TF取离散集合）、H\_update\_every、γ收缩映射系数（spread→γ的斜率范围）。
