...
WEIGHT_DIR = PROJECT_ROOT / 'Finance' / '02_核心代码' / '模型权重备份' / 'iterA5_ashare'
SMOKE_DIR = PROJECT_ROOT / 'Finance' / '02_核心代码' / '模型权重备份' / 'iterA5_ashare_smoke'
LOG_DIR = PROJECT_ROOT / 'logs' / 'iterA5_ashare'
LOG_PATH = LOG_DIR / 'iterA5_ashare_train.log'
FETCH_END_DATE = '2026-04-03'
RAW_FETCH_TIMEFRAMES = ['5m', '15m', '60m', '1d']
TRAIN_TIMEFRAMES = ['5m', '15m', '60m', '240m', '1d']
...
def prepare_and_validate():
    ...
    prepare_imported_ashare_data(
        ...
        target_timeframes=TRAIN_TIMEFRAMES,
    )
    coverage_report = build_market_coverage_report(
        ...
        timeframes=TRAIN_TIMEFRAMES,
        train_end=DEFAULT_TRAIN_END,
        val_end=DEFAULT_VAL_END,
        test_start=DEFAULT_TEST_START,
        training_subdir='ashare',
    )
    report_paths = write_coverage_reports(str(LOG_DIR), 'iterA5_ashare_coverage', coverage_report)
    ...
    if not coverage_report['asset_resolution']['sufficient']:
        raise RuntimeError(f'A-share coverage is insufficient for iterA5 formal training. See {report_paths["md_path"]}')
...
def main():
    ...
    smoke_args = build_namespace(
        ...
        batch_size=64,
        fast_full=False,
        save_prefix='iterA5_ashare_smoke',
        per_timeframe_train_cap={'5m': 512, '15m': 1024, '60m': 1024, '240m': 512, '1d': 512},
        max_files=len(smoke_assets) * len(TRAIN_TIMEFRAMES),
        resume=False,
    )
    formal_args = build_namespace(
        ...
        batch_size=256,
        fast_full=False,
        save_prefix='iterA5_ashare',
        per_timeframe_train_cap={'5m': 2048, '15m': 4096, '60m': 6144, '240m': 3072, '1d': 4096},
        max_files=None,
        resume=True,
    )
    ...
    with LOG_PATH.open(log_mode, encoding='utf-8', buffering=1) as log_file:
        ...
        log_file.write(f'train_timeframes={TRAIN_TIMEFRAMES}\n')
        log_file.write(f'mode={{"smoke_only": {args.smoke_only}, "formal_only": {args.formal_only}}}\n')
        log_file.flush()
        with contextlib.redirect_stdout(log_file), contextlib.redirect_stderr(log_file):
            if not args.formal_only:
                print('=== Smoke Check ===')
                print({'assets': smoke_assets, 'save_dir': str(SMOKE_DIR), 'timeframes': TRAIN_TIMEFRAMES})
                train(smoke_args)
            if not args.smoke_only:
                print('=== Formal Train ===')
                print({'assets': selected_assets, 'save_dir': str(WEIGHT_DIR), 'timeframes': TRAIN_TIMEFRAMES})
                train(formal_args)
        log_file.write('iterA5_runner_end\n')
        log_file.flush()

    print(f'iterA5 training finished. See log: {LOG_PATH}')
