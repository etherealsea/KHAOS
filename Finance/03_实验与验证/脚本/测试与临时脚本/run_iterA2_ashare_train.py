...
WEIGHT_DIR = PROJECT_ROOT / 'Finance' / '02_核心代码' / '模型权重备份' / 'iterA2_ashare'
SMOKE_DIR = PROJECT_ROOT / 'Finance' / '02_核心代码' / '模型权重备份' / 'iterA2_ashare_smoke'
LOG_DIR = PROJECT_ROOT / 'logs' / 'iterA2_ashare'
LOG_PATH = LOG_DIR / 'iterA2_ashare_train.log'
...
def prepare_and_validate():
    ...
    report_paths = write_coverage_reports(str(LOG_DIR), 'iterA2_ashare_coverage', coverage_report)
    if not coverage_report['asset_resolution']['sufficient']:
        raise RuntimeError(f'A-share coverage is insufficient for iterA2 formal training. See {report_paths["md_path"]}')
...
def main():
    ...
    smoke_args = build_namespace(
        ...
        batch_size=64,
        fast_full=True,
        save_prefix='iterA2_ashare_smoke',
    ...
    with LOG_PATH.open(log_mode, encoding='utf-8', buffering=1) as log_file:
        ...
        with contextlib.redirect_stdout(log_file), contextlib.redirect_stderr(log_file):
            ...
            if not args.smoke_only:
                ...
                train(formal_args)
        log_file.write('iterA2_runner_end\n')
        log_file.flush()

    print(f'iterA2 training finished. See log: {LOG_PATH}')
