...
WEIGHT_DIR = PROJECT_ROOT / 'Finance' / '02_核心代码' / '模型权重备份' / 'iterA4_ashare'
SMOKE_DIR = PROJECT_ROOT / 'Finance' / '02_核心代码' / '模型权重备份' / 'iterA4_ashare_smoke'
LOG_DIR = PROJECT_ROOT / 'logs' / 'iterA4_ashare'
LOG_PATH = LOG_DIR / 'iterA4_ashare_train.log'
FETCH_END_DATE = '2026-04-03'
RAW_FETCH_TIMEFRAMES = ['15m', '60m', '1d']
TRAIN_TIMEFRAMES = ['15m', '60m', '240m', '1d']
...
def prepare_and_validate():
    ...
    report_paths = write_coverage_reports(str(LOG_DIR), 'iterA4_ashare_coverage', coverage_report)
    ...
    if not coverage_report['asset_resolution']['sufficient']:
        raise RuntimeError(f'A-share coverage is insufficient for iterA4 formal training. See {report_paths["md_path"]}')
...
def main():
    ...
    smoke_args = build_namespace(
        ...
        batch_size=64,
        fast_full=True,
        save_prefix='iterA4_ashare_smoke',
        per_timeframe_train_cap={'15m': 1024, '60m': 1024, '240m': 512, '1d': 512},
    ...
    formal_args = build_namespace(
        ...
        batch_size=256,
        fast_full=False,
        save_prefix='iterA4_ashare',
        per_timeframe_train_cap={'15m': 4096, '60m': 5120, '240m': 3072, '1d': 4096},
        max_files=None,
        resume=True,
    )
    ...
    with LOG_PATH.open(log_mode, encoding='utf-8', buffering=1) as log_file:
        ...
        with contextlib.redirect_stdout(log_file), contextlib.redirect_stderr(log_file):
            ...
            if not args.smoke_only:
                ...
                train(formal_args)
        log_file.write('iterA4_runner_end\n')
        log_file.flush()

    print(f'iterA4 training finished. See log: {LOG_PATH}')
