...
DEFAULT_SCORE_TIMEFRAMES = "15m,60m,240m,1d"
DEFAULT_AUX_TIMEFRAMES = "5m"
...
FORMAL_PRESET = {
    ...
    "batch_size": 256,
    "arch_version": "iterA4_multiscale",
    "dataset_profile": "shortT_discovery_guarded_v3", # 确保使用包含 Focal Loss 机制的新版 profile
    "loss_profile": "shortT_discovery_guarded_v3",
    "constraint_profile": "teacher_feasible_discovery_v1",
    "score_profile": "iter13_precision_first",  # 使用专为 iter13 解除限制的 profile
    "score_timeframes": DEFAULT_SCORE_TIMEFRAMES,
    "aux_timeframes": DEFAULT_AUX_TIMEFRAMES,
    "split_scheme": DEFAULT_SPLIT_SCHEME,
    "split_labels": DEFAULT_SPLIT_LABELS,
...
def main() -> None:
    ...
    run_id = args.run_id or datetime.now().strftime(f"%Y%m%d_iter12_multiasset_{config['run_label']}_%H%M%S")
